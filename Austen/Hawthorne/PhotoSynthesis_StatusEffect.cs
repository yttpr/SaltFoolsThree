...
// Type: Hawthorne.PhotoSynthesis_StatusEffect
...
namespace Hawthorne
{
  public class PhotoSynthesis_StatusEffect : IStatusEffect, ITriggerEffect<IStatusEffector>
  {
    ...
    {
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnStatusTriggered), ((TriggerCalls) 40).ToString(), (object) caller);
    }

    public void OnTriggerDettached(IStatusEffector caller)
    {
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnStatusTriggered), ((TriggerCalls) 40).ToString(), (object) caller);
    }
    ...
    {
      if (!(args is IntValueChangeException valueChangeException) || valueChangeException.amount <= 0)
        return;
      valueChangeException.AddModifier((IntValueModifier) new MultiplyIntValueModifier(false, this.Amount));
    }
