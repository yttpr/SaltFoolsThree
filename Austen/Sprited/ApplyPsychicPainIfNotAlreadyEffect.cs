...
// Type: Austen.ApplyPsychicPainIfNotAlreadyEffect
...
namespace Austen
{
  public class ApplyPsychicPainIfNotAlreadyEffect : EffectSO
  {
    [SerializeField]
      ...
      out int exitAmount)
    {
      ...
      {
        if (targets[index1].HasUnit && !targets[index1].Unit.ContainsStatusEffect((StatusEffectType) PsyPain.Type, 0))
        {
